...
On a desktop computer with no GPU, we measure that the fast model takes on average <100 ms to encode 100 segments. The accurate model takes on average 20 seconds. Voxelizing these segments takes around 20 ms.
...
An intuition is that the accurate architecture, due to having 3D convolutions, would be able to more efficiently compress segment representations in the latent space. Our observations show that on average the accurate model keeps 3 latent dimensions unused, whereas the fast model keeps 1 unused (see Fig.~\ref{fig:fastvaccurate-features}). This goes in accordance with our expectations.\\
