...
Usually, LiDAR sensors do not produce ready-made point clouds of the local area. In this case, refer to the next subsections which illustrate ways in which raw sensor data can be transformed into SegMatch input.
...
This algorithm corrects the odometry by registering the new scans with the map using Iterative Closest Point (ICP). The laser and odometry measurements are finally combined using a pose-graph relaxation approach. The output is a 3D point coud which is becomes the input to the next step (i.e., segmentation).
...
An efficient method to extract patterns of matches from a list of potential matches is the \textit{Geometric Consistency} algorithm, based on random sample consensus (RANSAC, as presented in \cite{fischler1981random}).\\
